...
def find_train_match_prim(
    args: Tuple[
        np.ndarray,  # sampled_coords: (n_atoms, 3) - single generated sample
        np.ndarray,  # sampled_lattice: (6,) - lattice parameters
        np.ndarray,  # atom_types: (n_atoms,)
        np.ndarray,  # atom_mask: (n_atoms,) bool
        list,        # train_samples: list of dicts with training sample data
        Dict[str, Any],  # matcher_kwargs
    ],
) -> bool:
    """
    Check if a generated sample matches any training sample (primitive slab).

    This function compares a single generated sample against all training samples
    in the cache to detect if the model is memorizing training data.

    Args:
        args: Tuple containing:
            - sampled_coords: Generated atom coordinates (n_atoms, 3)
            - sampled_lattice: Generated lattice parameters (6,)
            - atom_types: Atomic numbers for each atom (n_atoms,)
            - atom_mask: Boolean mask for valid atoms (n_atoms,)
    ...
    Returns:
        True if the generated sample matches any training sample, False otherwise.
    """
    (
        sampled_coords,
        sampled_lattice,
    ...
    # Filter valid atoms
    valid_atom_types = atom_types[atom_mask]
    sampled_valid_coords = sampled_coords[atom_mask]

    # Create generated structure
    try:
        gen_lattice = Lattice.from_parameters(
            a=sampled_lattice[0],
            b=sampled_lattice[1],
            c=sampled_lattice[2],
            alpha=sampled_lattice[3],
            beta=sampled_lattice[4],
            gamma=sampled_lattice[5],
        )
    ...
    # Compare against all training samples
    for train_sample in train_samples:
        try:
            train_mask = train_sample["prim_slab_atom_mask"]
            train_atom_types = train_sample["prim_slab_atom_types"][train_mask]
            train_coords = train_sample["prim_slab_coords"][train_mask]
            train_lattice_params = train_sample["lattice"]

            train_lattice = Lattice.from_parameters(
                a=train_lattice_params[0],
                b=train_lattice_params[1],
                c=train_lattice_params[2],
                alpha=train_lattice_params[3],
                beta=train_lattice_params[4],
                gamma=train_lattice_params[5],
            )
            train_structure = Structure(
                lattice=train_lattice,
                species=train_atom_types.tolist(),
                coords=train_coords,
                coords_are_cartesian=True,
            )

            if matcher.fit(gen_structure, train_structure):
                return True
...
def find_train_match_slab(
    args: Tuple[
        np.ndarray,  # sampled_prim_slab_coords: (n_prim_slab_atoms, 3)
        np.ndarray,  # sampled_ads_coords: (n_ads_atoms, 3)
        np.ndarray,  # sampled_lattice: (6,)
        np.ndarray,  # sampled_supercell_matrix: (3, 3) or (9,)
        float,       # sampled_scaling_factor
        np.ndarray,  # prim_slab_atom_types: (n_prim_slab_atoms,)
        np.ndarray,  # ads_atom_types: (n_ads_atoms,)
        np.ndarray,  # prim_slab_atom_mask: (n_prim_slab_atoms,) bool
        np.ndarray,  # ads_atom_mask: (n_ads_atoms,) bool
    ...
    """
    Check if a generated sample matches any training sample (full slab).

    This function compares a single generated full slab against all training samples
    in the cache to detect if the model is memorizing training data.

    Args:
        args: Tuple containing all necessary data for slab comparison.
    ...
    (
        ...
        sampled_ads_coords,
        sampled_lattice,
        sampled_supercell_matrix,
        sampled_scaling_factor,
        prim_slab_atom_types,
        ads_atom_types,
        prim_slab_atom_mask,
        ads_atom_mask,
    ...
    # Assemble generated slab
    try:
        gen_system, gen_slab = assemble(
            generated_prim_slab_coords=sampled_prim_slab_coords,
            generated_ads_coords=sampled_ads_coords,
            generated_lattice=sampled_lattice,
            generated_supercell_matrix=sampled_supercell_matrix.reshape(3, 3),
            generated_scaling_factor=float(sampled_scaling_factor),
            prim_slab_atom_types=prim_slab_atom_types,
            ads_atom_types=ads_atom_types,
            prim_slab_atom_mask=prim_slab_atom_mask,
            ads_atom_mask=ads_atom_mask,
        )
        gen_structure = adaptor.get_structure(gen_slab)
    except Exception:
        return False
    ...
    for train_sample in train_samples:
        try:
            train_system, train_slab = assemble(
                generated_prim_slab_coords=train_sample["prim_slab_coords"],
                generated_ads_coords=train_sample["ads_coords"],
                generated_lattice=train_sample["lattice"],
                generated_supercell_matrix=train_sample["supercell_matrix"].reshape(3, 3),
                generated_scaling_factor=train_sample["scaling_factor"],
                prim_slab_atom_types=train_sample["prim_slab_atom_types"],
                ads_atom_types=train_sample["ads_atom_types"],
                prim_slab_atom_mask=train_sample["prim_slab_atom_mask"],
                ads_atom_mask=train_sample["ads_atom_mask"],
            )
            train_structure = adaptor.get_structure(train_slab)
